...
for(i in 1:length(lake_sites$field_site_id)) {  
  ...
  temp_lm_forecast_new <- dplyr::bind_rows(temp_lm_forecast, temperature)
  message(example_site, ' temperature forecast run')
  
}

my_model_id_new <- 'new_example_ID'

temp_lm_forecast_EFI_new <- temp_lm_forecast_new %>%
  filter(datetime > forecast_date) %>%
  mutate(model_id = my_model_id_new,
         reference_datetime = forecast_date,
         family = 'ensemble',
         parameter = as.character(parameter)) %>%
  select(datetime, reference_datetime, site_id, family, parameter, variable, prediction, model_id)
...
theme <- 'aquatics'
date <- temp_lm_forecast_EFI_new$reference_datetime[1]
forecast_name_1b <- paste0(temp_lm_forecast_EFI_new$model_id[1], ".csv")
forecast_file_1b <- paste(theme, date, forecast_name_1b, sep = '-')
forecast_file_1b
...
write_csv(temp_lm_forecast_EFI_new, file.path('Forecasts',forecast_file_1b))

neon4cast::forecast_output_validator(file.path('Forecasts',forecast_file_1b))

neon4cast::submit(forecast_file = file.path('Forecasts', forecast_file_1b),
                  ask = FALSE) # if ask = T (default), it will produce a pop-up box asking if you want to submit
```
