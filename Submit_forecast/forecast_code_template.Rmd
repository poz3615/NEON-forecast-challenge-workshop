...
for(i in 1:length(lake_sites$field_site_id)) {  
  ...
  forecasted_temperature <- param_df$beta1 + param_df$beta2 * lag(noaa_future_site$air_temperature)+ rnorm(n = 31, mean = 0, sd = sd(fit$residuals,na.rm=TRUE))
...
my_model_id <- 'feb6_add_uncert_example_ID'
